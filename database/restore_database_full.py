...
def restore_database():
    ...
    # Agent logs table (for clearing old log messages)
    ...
    """)
    print("  - 'agent_logs' table created (cleared old messages).")
    ...
    print("📝 Note: All old agent log messages have been cleared for a clean test environment.")

def clear_agent_logs():
    """
    Clears all agent log messages from the database.
    This is useful for cleaning up between test runs without doing a full database restore.
    """
    db_path = 'database/united_ops.db'
    ...
    print(f"Clearing agent logs from {db_path}...")
    
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    
    # Clear the agent_logs table
    ...
    print(f"✅ Cleared {deleted_count} agent log messages.")
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "clear-logs":
        ...
        print("Usage:")
        print("  python restore_database.py          # Full database restore (clears logs)")
        print("  python restore_database.py clear-logs # Clear only agent logs")
        print()
        restore_database() 